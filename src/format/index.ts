...
export default function init(context: vscode.ExtensionContext) {
	return Promise.all([rome_init(context), go_init(context), ruff_init(context), clang_init(context)]);
}

export function formattingSubscription() {
	return vscode.Disposable.from(rome_sub(), go_sub(), ruff_sub(), clang_sub());
}
