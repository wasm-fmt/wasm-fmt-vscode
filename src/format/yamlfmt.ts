...
import wasm from "@wasm-fmt/yamlfmt/wasm";
import { format, initSync } from "@wasm-fmt/yamlfmt/web";
...
export async function load() {
	...
	logger = vscode.window.createOutputChannel("wasm-fmt/yaml", { log: true });
	inited = new Promise((resolve, reject) => {
		vscode.workspace.fs.readFile(wasm_uri).then(
			(bits) => {
				initSync(bits);
				logger.info("yamlfmt inited");
				resolve();
			},
			(error) => {
				logger.error("failed to init yamlfmt", error);
				reject(error);
			},
		);
	});
