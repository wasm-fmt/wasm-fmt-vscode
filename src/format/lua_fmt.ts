...
import wasm from "@wasm-fmt/lua_fmt/wasm";
import { format, initSync } from "@wasm-fmt/lua_fmt/web";
...
export async function load() {
	...
	logger = vscode.window.createOutputChannel("wasm-fmt/lua", { log: true });
	inited = new Promise((resolve, reject) => {
		vscode.workspace.fs.readFile(wasm_uri).then(
			(bits) => {
				initSync(bits);
				logger.info("lua_fmt inited");
				resolve();
			},
			(error) => {
				logger.error("failed to init lua_fmt", error);
				reject(error);
			},
		);
	});
...
const selector: vscode.DocumentSelector = ["lua", { pattern: "**/*.lua", scheme: "file" }];
